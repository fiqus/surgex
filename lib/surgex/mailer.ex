defmodule Surgex.Mailer do
  use Bamboo.Mailer, otp_app: :surgex
end

defmodule Surgex.Email do
  import Bamboo.Email
  ...
  def set_password_email(surgeon, activation_url) when is_binary(activation_url) do
    ...
    |> from({"Surgex App", "info+surgex@fiqus.coop"})
    |> put_header("Reply-To", "no-reply@fiqus.coop")
    |> subject("Activar nueva cuenta")
    |> html_body("<strong>¡Hola #{name}!</strong><br/>
      Le damos la bienvenida al servicio de cirugía plástica del hospital de pediatría Surgex.<br/>
      <br/>
      ...
      ")
    |> text_body("¡Hola #{name}!\n
      Le damos la bienvenida al servicio de cirugía plástica del hospital de pediatría Surgex.\n
      \n
