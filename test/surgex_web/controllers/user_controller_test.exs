defmodule SurgexWeb.UserControllerTest do
  use SurgexWeb.ConnCase
  use SurgexWeb.AuthCase
  use Bamboo.Test

  alias Surgex.Accounts.{User, ActivationToken, RecoverToken}
  ...
  describe "recover user (GET)" do
    ...
    test "renders error 'WRONG_TOKEN' if the surgeon was not found", %{
      ...
      {:ok, _} = Surgex.Surgeries.delete_surgeon(surgeon)
      conn = get(conn, Routes.user_path(conn, :recover), data)
  ...
  describe "activate user (PUT)" do
    test "renders user when activation data and user are valid", %{conn: conn} do
      ...
      user = Surgex.Accounts.get_user!(user.id)
  ...
  defp fixture_activate_user() do
    {:ok, user} = Surgex.Accounts.create_user(%{is_admin: false})
